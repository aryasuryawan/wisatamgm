...
- [x] ~~Export laporan PDF~~ → **SELESAI** (2026-08-25): Dompdf terpasang; PDF receipt transaksi + lampiran email invoice + PDF rekap laporan.
- [x] ~~Export Excel~~ → **SELESAI** (2026-08-28): `maatwebsite/excel 4.0` + `App\Exports\ReportExport` (FromArray ShouldAutoSize WithStyles) + `ReportController@excel` `routes/web.php:reports.excel` `permission:reports.export` + tombol `Excel` `dusk="export-excel"` di `report/index.blade.php:8`. File `laporan-Ymd-Ymd.xlsx` 7-sheet sections (Ringkasan, Per Cabang, Kategori, Top Produk/Pelanggan, Kampanye) — test `Excel::raw` 7500 bytes OK.
